    ...
    public class Program
    {
        ...
        public static int Main(params string[] args)
        {
            ...
            var options = new OptionSet
            {
                ...
                _,
                {"keep-script=", "Keep files that match the patterns defined in the {<pattern_file>} from the current tree being visited (whitelist).", v=> keeps.Append(SafeReadText(v, "keep-script"))},
                _,
                {"r|remove=", "Remove files that match the {<pattern>} from the current tree being visited (blacklist).", v => removes.AppendLine(v)},
                _,
                {"remove-script=", "Remove files that match the patterns defined in the {<pattern_file>} from the current tree being visited (blacklist). ", v=> removes.Append(SafeReadText(v, "remove-script"))},
                _,
